...
# output of raw.data: "There are no rows with missing values greater than 20% in the data frame"
...
# test for normality (Shapiro-Wilk (SW) test) of continuous variables of baseline and outcome
normtest_var <-
  c(
    "age",
    "bmi_admit",
...
psModel <-
  glm(
    group ~ age + gender + race + bmi_admit + charlson_comorbidity_index + careunit + mnutric + sofa_24h
    + apacheii + vaso_flag + sepsis_flag + mechvent_flag + crrt_flag + wbc_avg_d1 + neutrophil_avg_d1
...
glm28.1 <-
  svyglm(
    sur_flag_28d ~ I(age / 5) + gender + race + bmi_admit + charlson_comorbidity_index + careunit
    + mnutric_c + sofa_24h + apacheii + vaso_flag + sepsis_flag + mechvent_flag + crrt_flag
    + I(wbc_avg_d1 / 2) + I(neutrophil_avg_d1 / 2) + I(lymphocyte_avg_d1 / 0.2) + I(albumin_avg_d1 / 0.2)
...
# glm28.2 equivalent to svyglm(sur_flag_28d ~ I(age/5) + bmi_admit + charlson_comorbidity_index
#                               + careunit + vaso_flag + crrt_flag + albumin_avg_d1 + group,
